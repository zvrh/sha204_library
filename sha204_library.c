...
extern void swi_init(uint8_t pin);

extern void swi_send_wakeup();

extern uint8_t swi_receive_bytes(uint8_t *buffer, uint8_t buffer_size);

extern uint8_t swi_send_bytes(const uint8_t *buffer, uint8_t buffer_size);

extern uint8_t swi_send_byte(uint8_t value);

void sha204_init(uint8_t pin) {
    swi_init(pin);
}

/* 	Puts a the ATSHA204's unique, 4-byte serial number in the response array returns an SHA204 Return code */
...
uint8_t sha204p_receive_response(uint8_t size, uint8_t *response) {
    ...
    ret_code = swi_receive_bytes(response, size);
...
uint8_t sha204p_send_command(uint8_t count, uint8_t *command) {
    ...
    return swi_send_bytes(command, count);
}
...
uint8_t sha204c_send_and_receive(uint8_t *tx_buffer, uint8_t rx_size, uint8_t *rx_buffer, uint8_t execution_delay,
